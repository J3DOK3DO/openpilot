...
def get_action_from_model(model_output: dict[str, np.ndarray], prev_action: log.ModelDataV2.Action,
    ...
    # Disable it for the Accord so model acceleration reaches the MVL planner unmodified.
...
def main(demo=False):
  ...
  # MVL Accord uses zero lateral action smoothing. Keep StarPilot model handling and
  # model compatibility, but remove its extra 0.1 s smoothing/delay for this car only.
  ...
  while True:
    ...
    # MVL's Accord timing starts from steerActuatorDelay + 0.2 s and then uses the
    # restored/learned liveDelay. Avoid a transient zero-delay model input before
    # StarPilot has received the first liveDelay message after process startup.
