"""Validated MVL longitudinal planner isolated to the Honda Accord 11G."""
...
def mvl_should_stop(v_ego, a_target):
  return bool(v_ego < 0.25 and a_target < 0.1)


def mvl_get_accel_from_plan(speeds, accels, t_idxs, action_t=DT_MDL):
  """Project short actuator delays through the stable 0.3 second plan point."""
...
class MVLAccordLongitudinalPlanner:
  def __init__(self, CP, init_v=0.0, init_a=0.0, dt=DT_MDL, mpc=None):
    self.CP = CP
    self.mpc = mpc if mpc is not None else MVLAccordLongitudinalMpc(dt=dt)
    self.fcw = False
    self.dt = dt
  ...
  def update(self, sm, starpilot_toggles=None):
    # Only the StarPilot speed target is consumed below, and only as a reduction
    # from the stock cruise setpoint. All shared longitudinal tuning is bypassed.
    del starpilot_toggles
    ...
    v_cruise = min(stock_v_cruise, starpilot_v_cruise) if np.isfinite(starpilot_v_cruise) and starpilot_v_cruise >= 0.0 else stock_v_cruise
    ...
    accel_clip = limit_accel_in_turns(v_ego, steering_angle, accel_clip, self.CP)
    ...
    previous_a_desired = self.a_desired
    self.a_desired = float(np.interp(self.dt, CONTROL_N_T_IDX, self.a_desired_trajectory))
    self.v_desired_filter.x += self.dt * (self.a_desired + previous_a_desired) / 2.0
    ...
    for index in range(2):
      accel_clip[index] = float(np.clip(
