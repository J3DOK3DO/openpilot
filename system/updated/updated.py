...
# Target checkouts use several historical AGNOS manifest layouts. The updater
# that is currently running must be able to finalize any of those layouts.
...
def get_agnos_manifest_path(basedir: str | Path) -> str:
  base = Path(basedir)
  for relative_path in AGNOS_MANIFEST_PATHS:
    manifest = base / relative_path
    if manifest.is_file():
      return str(manifest)
  raise FileNotFoundError(
    f"no agnos.json found in {base}; tried: {', '.join(AGNOS_MANIFEST_PATHS)}"
  )
