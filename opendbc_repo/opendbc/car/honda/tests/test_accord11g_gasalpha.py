...
def test_persistence_is_nonblocking_bounded_and_preserves_non_accord_writes():
  class FakeParams:
    def __init__(self):
      self.values = []
    ...
    def put(self, *args, **kwargs):
      raise AssertionError("synchronous persistence is forbidden")

  params = FakeParams()
  persist_honda_learned_params_nonblocking(params, 1.25, 0.875)
  assert params.values == [("HondaGasFactorParams", 1.25), ("HondaWindFactorParams", 0.875)]

  for _ in range(100):
    persist_honda_learned_params_nonblocking(params, 1.25, 0.875, 0.2)
  assert len(params.values) == 302
  assert params.values[-3:] == [
    ("HondaGasFactorParams", 1.25), ("HondaWindFactorParams", 0.875), ("HondaGasAlphaParams", 0.2),
  ]
