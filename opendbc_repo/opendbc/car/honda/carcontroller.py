...
def apply_accord11g_gasalpha(gas_force_without_alpha: float, gasalpha: float, retained_brake_active: bool) -> float:
  # NATIVE-13 remains authoritative until its retained negative correction reaches zero.
  if retained_brake_active:
    return min(gas_force_without_alpha, 0.0)
  return gas_force_without_alpha + gasalpha


def get_honda_bosch_gas_command(gas_pedal_force: float, gas_factor: float, accord11g: bool) -> float:
  if accord11g:
    # Accord NATIVE-16 uses a true zero-centered propulsion domain. Other Bosch Hondas retain
    # their exact NATIVE-15 lookup below.
    return float(np.interp(
      gas_pedal_force * gas_factor,
...
def persist_honda_learned_params_nonblocking(params, gas_factor, wind_factor, gasalpha=None):
  # Preserve the exact two-write behavior for non-Accord controllers.
  ...
  def update(self, CC, CS, now_nanos, starpilot_toggles):
        ...
        if self.CP.carFingerprint in HONDA_BOSCH:
          ...
          if self.CP.carFingerprint not in HONDA_BOSCH_RADARLESS:
            ...
            if self.mvl_accord_mode:
              retained_brake_active = self.mvl_brake_pid.i < 0.0
              self.bosch_gasalpha = update_accord11g_gasalpha(
                ...
                retained_brake_active,
                actuators.experimentalToAccTransition,
              )
              gas_pedal_force = apply_accord11g_gasalpha(
                gas_pedal_force, self.bosch_gasalpha, retained_brake_active,
              )

            if actuators.longControlState == LongCtrlState.pid and not CS.out.gasPressed:
              gas_error = (accel if self.mvl_accord_mode else self.accel) - CS.out.aEgo

              gas_learning_active = gas_pedal_force > (0.0 if self.mvl_accord_mode else min_gas)
              if gas_error != 0.0 and gas_learning_active:
          ...
          self.gas = get_honda_bosch_gas_command(gas_pedal_force, self.bosch_gas_factor, self.mvl_accord_mode)
    ...
    if self.frame > 0 and self.frame % 6000 == 0:
      persist_honda_learned_params_nonblocking(
        self.param_store, self.bosch_gas_factor, self.bosch_wind_factor, self.bosch_gasalpha if self.mvl_accord_mode else None,
      )
