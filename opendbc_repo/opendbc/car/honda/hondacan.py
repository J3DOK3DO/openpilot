...
def create_acc_hud(packer, bus, CP, enabled, pcm_speed, pcm_accel, hud_control, hud_v_cruise, is_metric, acc_hud):
  ...
  if CP.carFingerprint in HONDA_BOSCH_CANFD:
    ...
    # neither lead nor braking is present matches the validated CAN-FD payload shape.
  ...
  if CP.carFingerprint in HONDA_BOSCH:
    ...
    # CAN-FD replacement radar keeps FCW/CMBS enabled. Preserve existing Dom behavior elsewhere.
...
def create_canfd_5hz_radar_messages(packer, bus, radar_ref_cntr):
  radar_lead_values = {
    ...
    # Initial takeover uses the road-validated stock-like idle lane state.
    'LEFT_LANE': 0,
    'RIGHT_LANE': 0,
    'LANE_PATH_LENGTH': 6,
  }
